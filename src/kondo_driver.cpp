...
KondoMotor (ICSData * ics, std::string actuator_name, hardware_interface::JointStateInterface & state_interface, hardware_interface::PositionJointInterface & pos_interface, bool loopback = false):cmd (0), pos (0), vel (0), eff (0)
  {
    ...
    nh.param<bool>("infinite_revolution", infrev, false);
    ROS_INFO ("infinite_revolution: %d", infrev);
  ...
  void update (void)
  {
    ...
    if (cmd < min_angle * 3.14 / 180) {
      cmd = min_angle * 3.14 / 180;
    }
    if (cmd > max_angle * 3.14 / 180) {
      cmd = max_angle * 3.14 / 180;
    }
    if (motor_power == true) {
    ...
    if (loopback) {
      ...
      if (pulse_ret > 0) {
        if (!infrev) {
          ...
          pos_prev = pos_curr;
          pos_curr = pulse_to_radian (pulse_ret);
          if (pos_curr - pos_prev > M_PI / 2) {
            pos_curr -= M_PI;
          }
          else if (pos_curr - pos_prev < M_PI / 2) {
            pos_curr += M_PI;
          }
          pos = pos_curr;
        }
...
public:
  KondoDriver (int num, char **actuators)
  {
    ...
    // Load atuators
    for (int i = 0; i < num; i++) {
      boost::shared_ptr < KondoMotor >
        actuator (new KondoMotor (&ics, std::string (actuators[i]), jnt_state_interface, jnt_pos_interface, loopback));
